...
为了方便我们操作，Sentinel官方提供了UI控制台，我们可以直接到[GitHub](https://github.com/alibaba/Sentinel/releases)上下载相关jar包，如果网速不行，也可以使用[备用地址](https://gongsl.lanzout.com/izSNMw3kq0b)进行下载。然后直接使用`java -jar sentinel-dashboard-1.8.1.jar`命令进行启动即可，默认使用的端口是8080，控制台默认用户名和密码都是==sentinel==。
...
线程属性配置完成之后，启动访问，此时`/order/update`资源的访问都是没有问题的：
...
![image-20220211104515552](https://cdn.jsdelivr.net/gh/gongcqq/FigureBed@main/Image/Typora/20220211111052.png) 
...
这里部署的Seata其实就是上文中提到的事务协调者(TC)，我们可以直接到GitHub上进行[下载](https://github.com/seata/seata/releases/download/v1.4.2/seata-server-1.4.2.tar.gz)，如果网速不好，也可以使用[备用地址](https://gongsl.lanzout.com/i5hNGwjbu9g)进行下载。下载完成之后，使用`tar -zxvf seata-server-1.4.2.tar.gz `命令进行解压即可。
...
然后编写一个Try、Confirm、Cancel相关方法对应的[AccountTCCService](https://gitee.com/gongcqq/others/raw/master/seata/AccountTCCService.java)接口：
...
之后就是编写以上接口对应的[AccountTCCServiceImpl](https://gitee.com/gongcqq/others/raw/master/seata/AccountTCCServiceImpl.java)实现类：
...
![image-20211118232817383](https://cdn.jsdelivr.net/gh/gongcqq/FigureBed@main/Image/Typora/20211118234854.png) 

而且account_freeze_tbl表中还会多出下面这样一条数据：

![image-20211118232341494](https://cdn.jsdelivr.net/gh/gongcqq/FigureBed@main/Image/Typora/20211118234902.png) 

> **说明**：表里的`state`字段的值为2就说明，TCC模式在回滚的时候确实执行了cancel方法。
...
<img src="https://cdn.jsdelivr.net/gh/gongcqq/FigureBed@main/Image/Typora/20211118234910.png" alt="image-20211118233229764" style="zoom: 70%;" /> 

> **说明**：由于Saga模式的缺点明显，并且使用场景并不多，所以这里就不再进行实战演示了。

##### 9.4.5 四种模式对比

![image-20211118234142239](https://cdn.jsdelivr.net/gh/gongcqq/FigureBed@main/Image/Typora/20211118234919.png) 

> **说明**：可以直接点击[seata-demo](https://gitee.com/gongcqq/others/attach_files/884801/download/seata-demo.zip)下载截止到目前为止Seata章节的项目代码。
