...
#### 2.6 IDEA合并代码的Merging问题

在使用IDEA合并不同分支的代码后，明明冲突都已经解决完了，但IDEA右下角还是出现"**Merging**"字样，旁边还有黄色警告，解决办法就是依次使用`git add`和`git commit`命令，然后警告就会消失了，这时候也可以`push`了。

如果别的分支过来的文件有的是不想要的，也可以在`git add`后在回滚的时候删除掉。

![image-20221003150524112](D:\Program Files (x86)\Typora\images\工作中遇到的技术总结\image-20221003150524112.png) 
