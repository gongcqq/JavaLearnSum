...
- 可以使用这个注解来做配置绑定，这个注解可以将`properties`文件或者`yaml`文件中的数据绑定到javaBean中；
- `@ConfigurationProperties`一般搭配`@Component`或者`@EnableConfigurationProperties`来使用。因为只有容器中的组件才能使用类似配置绑定等功能，所以必须使用`@Component`等注解把javaBean注入到容器中；
- `@ConfigurationProperties`注解一般是放在实体类上的，然后可以使用该注解中的**prefix**属性来和配置文件中相同前缀的数据进行绑定。但是最好不要使用`@ConfigurationProperties(prefix = "user")`这种写法，因为前缀如果是"user"而实体类中正好也有一个“name”属性的话，那么在进行数据绑定的时候，不管配置文件中针对该属性配置的值是什么，这个属性获取到的都是我们的电脑用户名；
